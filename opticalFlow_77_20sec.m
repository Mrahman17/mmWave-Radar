function [ ] = opticalFlow_77_20sec( fNameIn, fNameOut, fnameBin )
...
%% only 20 sec
    Data2 = Data(1:end/3,:);
    Np = floor(size(Data2(:,1),1)/NTS); % #of pulses
...
%% No IQ Correction
for ii = 1:4
    Colmn = floor(length(Data2(:,1))/NTS);
    rawData(:,:,ii) = reshape(Data2(:,ii),NTS,Colmn);
    fftRawData(:,:,ii) = fftshift(fft(rawData(:,:,ii)),1);
    rp((1:NTS/2),:,ii) = fftRawData(((NTS/2+1):NTS),:,ii); %range profile,color space
end
%% MTI Filter
...
%% Optical Flow
    ...
    duration = dT * length(Data2)/NTS;
    freqaxis = linspace(0, prf/2, size(sx3,1));
%     imwrite(rescale(abs(sx1),0,255),fNameOut);
    fig = figure('units','normalized','outerposition',[.1 .3 .5 .5],'Visible','off');
    colormap(jet(256));
