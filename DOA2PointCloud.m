...
% Read the DOA video and extract frames
%vidObj = VideoReader('/mnt/HDD03/DOA2PCloud/doa/1sub_back_and_forth.avi');

        load('/mnt/HDD03/DOA2PCloud/RDC-walk.mat')
        numTX = 2;
        numRX = 4;
...
%                                 A = squeeze(sum(temp,2));
             ...
             %% Optical Flow Weightening
                ...
                RA_frame=20*log10(abs(raoaFrame) .* magnitudes ./ max(abs(raoaFrame(:))));
...
%         end
%         
%         fNameOut='/mnt/HDD03/DOA2PCloud/DOA-VICON-walk.avi';
%         
...
%%  CA-CFAR params
    ...
    figure('Visible','off')%,
     [RA_MASK, cfar_ranges, cfar_ang] = ca_cfar(RA_frame, numGuard, numTrain, P_fa, SNR_OFFSET);
        if ~isempty(cfar_ranges)
                cfar_bins(1,j) = min(cfar_ranges);
                cfar_bins(2,j) = max(cfar_ranges);
        else
                cfar_bins(1,j) = 17;
                cfar_bins(2,j) = 35;
        end
      scatter(cfar_ang,cfar_ranges); xlim([0 121]); ylim([0 73]);grid on; grid minor;
 ...
 fNameOut='/mnt/HDD03/DOA2PCloud/RDC-walk_high_guard.avi';
    writerObj = VideoWriter(fNameOut);
    writerObj.FrameRate = fps;
    open(writerObj);
