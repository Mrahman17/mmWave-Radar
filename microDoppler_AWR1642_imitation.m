function [] = microDoppler_AWR1642_imitation(fname, fOut)
    % read .bin file
    fid = fopen(fname,'r');
    ...
    NoC = 255;%128; % Number of chirp loops
    ...
    if numTX == 2
       ...
       if size(LVDS,2) > size(LVDS0,2)
               LVDS(:,end) = [];
               tmp = 1; % frame drop
               Data = zeros((numChirps-1)*numADCSamples, numRX*numTX);
       else
    ...
    if tmp
             rawData = reshape(Data,NTS,numChirps-1, numRX*numTX); 
    else
...
% fs=1e9/2;%due to complex sampling; sampling at 500 MHz I/Q
% dt=1/fs; 
% ft=0:dt:(M1-1)*dt;
% ft_idx_range=1:M1;
% ft=ft(ft_idx_range);
% range=(ft-surf_time)*3e8/sqrt(3.15)/2;